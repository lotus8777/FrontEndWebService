using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;

namespace FE.Model.Hrp275
{

    [Table("hzyb_jsjl")]
    public  class HzybJsjl
    {
        [Key,Column(Order = 1)]
        public string Fphm { get; set; }
        [Key, Column(Order = 2)]
        public decimal Jzlx { get; set; }
        public string Yylsh { get; set; }
        public string Zxlsh { get; set; }
        public string Jslsh { get; set; }
        public DateTime? Jsrq { get; set; }
        public string Czgh { get; set; }
        public string Yllb { get; set; }
        public string Rylb { get; set; }
        public string Bjgb { get; set; }
        public string Yldy { get; set; }
        public decimal Zfpb { get; set; }
        public DateTime? Zfrq { get; set; }
        public string Ywzq { get; set; }
        public string Zfzq { get; set; }
        public string Ybkh { get; set; }
        public string Grbh { get; set; }
        public decimal? Zjje { get; set; }
        public decimal? Dnzh { get; set; }
        public decimal? Lnzh { get; set; }
        public decimal? Lnzf { get; set; }
        public decimal? Ybjj { get; set; }
        public decimal? Xjje { get; set; }
        public decimal? Xjzf { get; set; }
        public decimal? Zlje { get; set; }
        public decimal? Zfje { get; set; }
        public decimal? Cxzf { get; set; }
        public decimal? Dnye { get; set; }
        public decimal? Lnye { get; set; }
        public decimal? Zycs { get; set; }
        public decimal? Xjlj { get; set; }
        public decimal? Dnlj { get; set; }
        public decimal? Tclj { get; set; }
        public decimal? Qflj { get; set; }
        public decimal? Zbjj { get; set; }
        public decimal? Knjz { get; set; }
        public decimal? Sczl { get; set; }
        public decimal? Sczf { get; set; }
        public decimal? Lmjj { get; set; }
        public decimal? Bjjj { get; set; }
        public decimal? Tcjj { get; set; }
        public decimal? Qfzf { get; set; }
        public string Zflsh { get; set; }
        public string Jzlsh { get; set; }
        public decimal? Lnzl { get; set; }
        public string Tsxx { get; set; }
        public string Yybh { get; set; }
        public string Cqh { get; set; }
        public decimal? Xjzfzl { get; set; }
        public decimal? Xjzfzf { get; set; }
        public decimal? Lnzhzl { get; set; }
        public decimal? Lnzhzf { get; set; }
        public decimal? Ybzhzf { get; set; }
        public decimal? Bjjjzf { get; set; }
        public decimal? Bfze { get; set; }
        public decimal? Bfzlzf { get; set; }
        public decimal? Zflj { get; set; }
        public decimal? Lgrjj { get; set; }
        public decimal? Yycd { get; set; }
        public decimal? Qtjj { get; set; }
        public decimal? Yycdfy { get; set; }
        public decimal? Qtjjzc { get; set; }
        public string Qhdm { get; set; }
        public string Sybxbz { get; set; }
        public string Sybxicd { get; set; }
        public string Sybxbfz { get; set; }
    }
}
